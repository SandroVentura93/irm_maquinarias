...
@section('content')
<div class="container">
    <h2 class="mb-4">Reporte Semestral</h2>
    <form method="GET" action="{{ route('reportes.semestral') }}" class="row g-3 mb-4">
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            ...
            <select name="semester" id="semester" class="form-select">
                <option value="1" {{ $semester == 1 ? 'selected' : '' }}>Enero - Junio</option>
                <option value="2" {{ $semester == 2 ? 'selected' : '' }}>Julio - Diciembre</option>
            </select>
        </div>
    ...
    <div class="mb-4">
        <canvas id="graficoSemestral" width="600" height="300"></canvas>
        <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
        <script>
        window.addEventListener('DOMContentLoaded', function() {
            var ctx = document.getElementById('graficoSemestral').getContext('2d');
            var labels = [
                @foreach($months_data as $month)
                    '{{ $month['name'] }}',
                @endforeach
        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-body text-center">
                    <h5 class="mb-3">Totales del Semestre</h5>
    ...
    <div class="mb-4">
    <a href="{{ route('reportes.semestral.pdf', ['year' => $year, 'semester' => $semester]) }}" class="btn btn-danger me-2">Exportar PDF</a>
    <a href="{{ route('reportes.semestral.excel', ['year' => $year, 'semester' => $semester]) }}" class="btn btn-success">Exportar Excel</a>
    </div>
    @endif
</div>
@endsection
